...
# https://github.com/CoffeaTeam/coffea/pull/329
...
def export1d_coffea(hist):
    ...
    out._fEntries = out._fTsumw = sumw[1:-1].sum()
...
def export1d_hist(h):
    ...
    Creating a coffea histogram, filling, and writing to a file::
        import coffea, uproot, numpy
        h = hist.Hist(hist.axis.Regular(20, 0, 1, name="var"))
        h.fill(var=numpy.random.normal(size=100))
        fout = uproot.create('output.root')
        fout['myhist'] = export1d_hist(h)
        fout.close()
    """
    import hist

    (axis,) = h.axes
    assert isinstance(axis, (hist.axis.Regular, hist.axis.Variable))

    sumw = h.view(flow=True)["value"]
    sumw2 = h.view(flow=True)["variance"]
    edges = axis.edges
    ...
    out._fXaxis._fName = axis.name
    out._fXaxis._fTitle = axis.label
    if not isinstance(axis, hist.axis.Regular):
        out._fXaxis._fXbins = edges.astype(">f8")
